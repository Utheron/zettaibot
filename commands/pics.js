...
exports.run = async (bot, message, args) => {
    ...
    if (fn.isEmpty(args) || isNaN(args)) {
        var array = []
        for (var i in pics) {
            array.push(`${i}. *${pics[i].category}* - **${pics[i].name}** by ${pics[i].author}`);
        }
        
        // RichEmbed for question list
        const embed = new Discord.RichEmbed()
        .setColor(color.info)
        .setTitle(`**Quizz format *__image__* **`)
    ...
    if (fn.isInteger(args)) {
        ...
        const deck = pics[args].questions;
        const hand = deck.sort(() => Math.floor(Math.random() * pics[args].questions.length))
        ...
        while (i < limit) {
            ...
            .attachFile(`./src/pics/${hand[i].img}`)
